...
namespace Infrastructure.Identity
{
    public class UserService : IUserService
    {
        ...
        private readonly IMultiTenantContextAccessor<ABCSchoolTenantInfo> _tenantContextAccesor;

        public UserService(UserManager<ApplicationUser> userManager, RoleManager<ApplicationRole> roleManager, ApplicationDbContext context, IMultiTenantContextAccessor<ABCSchoolTenantInfo> tenantContextAccesor)
        {
            _userManager = userManager;
            _roleManager = roleManager;
            _context = context;
            _tenantContextAccesor = tenantContextAccesor;
        }
        ...
        public async Task<string> AssignRolesAsync(string userId, UserRolesRequest request)
        {
            ...
            {
                var adminUserCount = (await _userManager.GetUsersInRoleAsync(RoleConstants.Admin)).Count;
                if (userInDb.Email == TenancyConstants.Root.Email)
                {
                    if (_tenantContextAccesor.MultiTenantContext.TenantInfo.Id == TenancyConstants.Root.Id)
                    {
        ...
        public async Task<bool> IsPermissionAssignedAssync(string userId, string permission, CancellationToken ct = default)
        {
